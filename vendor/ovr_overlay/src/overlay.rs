// OpenVRのIVROverlay操作をRustのResultベースAPIへ変換する。
...
// Contextに紐づくオーバーレイ操作窓口。生ポインタはこの型の内側へ閉じ込める。
...
impl<'c> OverlayManager<'c> {
    ...
    // 既にOpenVR側にあるオーバーレイをキーから探す。
    pub fn find_overlay(&mut self, key: &str) -> Result<OverlayHandle, EVROverlayError> {
        let mut handle = sys::VROverlayHandle_t::default();
        let err = unsafe {
            self.inner
                .as_mut()
                .FindOverlay(key.as_ptr().cast(), &mut handle)
        };

        EVROverlayError::new(err)?;
        Ok(OverlayHandle(handle))
    }
    // 新しいオーバーレイをOpenVRへ登録し、以後の操作に使うハンドルを返す。
    ...
    // ShowOverlay/HideOverlayをbooleanで扱えるようにする。
    ...
    // HMDなどのTrackedDeviceを基準にした相対変換を設定する。
    pub fn set_transform_tracked_device_relative(
        &mut self,
        overlay: OverlayHandle,
        tracked_device_index: u32,
        tracked_device_to_overlay: &Matrix3x4,
    ) -> Result<(), EVROverlayError> {
        let tracked_device_to_overlay: &sys::HmdMatrix34_t = tracked_device_to_overlay.into();
        let err = unsafe {
            self.inner
                .as_mut()
...
// OpenVRが返すオーバーレイハンドルを型で包み、取り違えを減らす。
